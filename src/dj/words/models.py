...
class Word(models.Model):
    language = models.CharField(max_length=7, choices=constants.LANGUAGES.items())
...
class WordInfo(models.Model):
    ...
    language = models.CharField(
        max_length=7, choices=constants.LANGUAGES.items()
    )
    ...
    def json(self):
        return {
            'word': self.word.full,
            'word_language': constants.LANGUAGES[self.word.language],
            'language': constants.LANGUAGES[self.language],
            'meanings': [word.full for word in self.meanings.all()],
            'antonyms': [word.full for word in self.antonyms.all()],
            'synonyms': [word.full for word in self.synonyms.all()],
            'text': self.text,
        }
